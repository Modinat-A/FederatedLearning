...
federated_train_data=[user2_train,user4_train,user1_train]

"""## **Federated Learning with Tensorflow -Fed Avg**"""

def create_keras_model(n_timesteps,n_features=10,n_outputs=11):
  return keras.Sequential([Conv1D(filters=64, kernel_size=3, input_shape=(n_timesteps,n_features)),
...
def model_fn():
  # We _must_ create a new model here, and _not_ capture it from an external TFF will call this within different graph contexts.
...
test_metrics = evaluation(model_weights, [user3_train])
...
prox_state = iterative_process_prox.initialize()
